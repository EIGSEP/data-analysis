...
for ax, clip in zip(axes, (None, (-33, 4))):
    for c in tones:
    ...
    ax.set_title("autoscale" if clip is None else "clipped at $-33$ dB (current)", fontsize=9)
axes[0].set_ylabel("Injected tone power rel. $0^\\circ$ [dB]", fontsize=8)
fig.tight_layout()
out = Path(__file__).parent / "58_autoscale.png"

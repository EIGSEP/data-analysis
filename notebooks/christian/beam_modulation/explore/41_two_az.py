"""Two azimuths, 90 deg apart, both from the well-behaved first half.
Same 23-tone selection as 34_three_az (reference rotation 18)."""
...
for c in [c for c in ch[(ch%16)==8] if 50<fr[c]<200]:
    ...
    if np.nanmedian(np.abs(np.diff(fold(C.db(np.nanmedian(d4[:,nb],axis=1)),a18,b18),2)))<0.06 and tc>=3.0:
        TONES.append(c)
fq=np.array([fr[c] for c in TONES]); print(f"{len(TONES)} tones, {fq.min():.0f}-{fq.max():.0f} MHz")
...
for ax,ri in zip(axes,[0,18]):
    a,b=SEGS[ri]; a0=np.nanmedian(az[a:b]); dep=[]
    for c in TONES:
        p=fold(C.db(d4[:,c]),a,b); ax.plot(Ec,p,c=sm.to_rgba(fr[c]),lw=.9,alpha=.95)
    ...
    print(f"  az {a0:+5.0f} deg (rot {ri:2d}, t={tm[a]:4.1f} min): depth {min(dep):4.1f} to {max(dep):4.1f} dB")
axes[0].set_ylabel("Power relative to $0^\\circ$ [dB]",fontsize=9)
cb=fig.colorbar(sm,ax=axes,pad=.02,fraction=.04); cb.set_label("Frequency [MHz]",fontsize=9)
cb.ax.tick_params(labelsize=8)
fig.savefig("41_two_az.png",dpi=200,bbox_inches="tight")
print(f"{NF[0]} samples flagged"); print("saved 41_two_az.png")

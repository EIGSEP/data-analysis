"""Three azimuths from the well-behaved first half of the scan."""
...
for ax,ri in zip(axes,[0,14,28]):
    a,b=SEGS[ri]; a0=np.nanmedian(az[a:b]); dep=[]
    for c in TONES:
        p=fold(C.db(d4[:,c]),a,b); ax.plot(Ec,p,c=sm.to_rgba(fr[c]),lw=.85,alpha=.95)
...
fig.savefig("40_clean_trio.png",dpi=200,bbox_inches="tight")
print(f"{NF[0]} samples flagged ({NF[0]/(3*len(TONES)*128)*100:.2f}%)")
print("saved 40_clean_trio.png")
